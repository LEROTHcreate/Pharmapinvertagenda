...
/**
 * Card de connexion (client) — possède l'état `success` pour que l'overlay de
 * succès soit rendu en ENFANT DIRECT de la card `relative` (et non dans le
 * `<form>`, dont le wrapper `animate-fade-up` crée un bloc englobant qui
 * confinait l'overlay à la zone des champs). Résultat : l'overlay couvre
 * toute la card, plus aucun texte (titre, lien) visible derrière.
 */
  ...
  return (
    <div className="relative overflow-hidden rounded-[28px] border border-white/60 bg-white/80 px-8 py-10 shadow-[0_30px_80px_-20px_rgba(76,29,149,0.35),0_8px_24px_-12px_rgba(76,29,149,0.18)] backdrop-blur-2xl sm:px-10 sm:py-12">
      ...
      <div className="animate-fade-up delay-225 mt-8">
        ...
        >
          <LoginForm success={success} onSuccess={() => setSuccess(true)} />
        </Suspense>
      </div>
      ...
      {/* Overlay de succès — enfant DIRECT de la card → couvre toute la card */}
