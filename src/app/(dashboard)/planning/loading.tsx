      ...
      <div className="animate-pulse space-y-5">
        ...
        <div className="overflow-hidden rounded-xl border border-border/70 bg-card">
          {Array.from({ length: 12 }).map((_, i) => (
            <div
              key={i}
              className="grid grid-cols-[64px_repeat(8,1fr)] border-b border-border/60 last:border-b-0"
            >
              <div className="border-r border-border/60 bg-muted/40/60 p-2">
                <div className="h-3 w-10 rounded bg-zinc-200/70" />
              </div>
              {Array.from({ length: 8 }).map((__, j) => (
                <div key={j} className="border-r border-border/60 p-2 last:border-r-0">
