/**
 * Seed "Pharmacie du Pin Vert" — pharmacie de démo / compte de test.
 *
 *  - 1 pharmacie "Pharmacie du Pin Vert" (Marseille) avec son logo /logo.png
 *  - 17 employés représentatifs (multi-statuts)
 *  - 2 utilisateurs (admin Thorel Nicolas + employé Stéphane)
 *  - 2 semaines de planning réaliste (couverture 08:30 → 20:00 staggered)
 *  - Quelques absences planifiées + demandes pendantes
 *
 *  Idempotent : peut être relancé plusieurs fois (purge avant insert).
 *
 *  Usage standalone :
 *    npx tsx prisma/seeds/pin-vert.ts
 *
 *  Usage via le router :
 *    npm run db:seed             (défaut → Pinvert)
...
// Les noms de famille sont des placeholders (à compléter via /equipe).
...
/**
 * Seed la pharmacie démo "Pin Vert". Idempotent : purge tout avant insert.
 * À utiliser pour réinitialiser l'environnement de test.
 */
export async function seedPinVert(prisma: PrismaClient): Promise<void> {
  ...
  console.log("→ Création de la pharmacie de démo (Pin Vert)...");
  ...
  const pharmacy = await prisma.pharmacy.create({
    data: {
      ...
      minStaff: 4,
      // Logo historique (croix verte + caducée) servi depuis /public/logo.png
      logoUrl: "/logo.png",
    },
  });
  ...
  console.log(
    `✓ Seed Pin Vert terminé : ${employees.length} employés, ${entries.length} créneaux planning`
  );
}

// Permet de lancer le fichier directement : `npx tsx prisma/seeds/pin-vert.ts`
